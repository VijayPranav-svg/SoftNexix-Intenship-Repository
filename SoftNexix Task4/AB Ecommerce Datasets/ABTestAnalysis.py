...
if 'session_duration' in Database_After_Imputation.columns:
    Vpcontroltime = Database_After_Imputation[Database_After_Imputation['group'] == 'control']['session_duration']
    VpcontrolTreatmt = Database_After_Imputation[Database_After_Imputation['group'] == 'treatment']['session_duration']


    stat_c, p_c = shapiro(Vpcontroltime)
    stat_t, p_t = shapiro(VpcontrolTreatmt)
    print("\n I am doing The Normality Check (Shapiro-Wilk) Here:")
    print(f"Control p={p_c:.4f}, Treatment p={p_t:.4f}")
    if p_c > 0.05 and p_t > 0.05:
        print(" Both groups appear normally distributed.")
    else:
        print(" At least one group not normally distributed. Consider non-parametric tests (e.g., Mann-Whitney U).")


    stat_l, p_l = levene(Vpcontroltime, VpcontrolTreatmt)
    print(f" I am Doing The Levene's Test for Equal Variance p={p_l:.4f}")
    if p_l > 0.05:
        print(" Equal variances assumed.")
    else:
        print(" Variances differ — consider Welch's T-test.")
...
if 'session_duration' in Database_After_Imputation.columns:
    ...
    t_stat, p_val = stats.ttest_ind(VpcontrolTreatmt, Vpcontroltime)
...
print(" \n I am Doing The Pricing Test: premium vs standard revenue") 
if {'plan_type', 'revenue'}.issubset(Database_After_Imputation.columns):
    revenue_premium = Database_After_Imputation[Database_After_Imputation['plan_type'] == 'premium']['revenue']
    revenue_standard = Database_After_Imputation[Database_After_Imputation['plan_type'] == 'standard']['revenue']
    t_stat_price, p_price = stats.ttest_ind(revenue_premium, revenue_standard)
    print(f"\nPricing Test: Does premium plan increase revenue?\nP-value: {p_price:.4f}")
else:
    print("\nSkipping Pricing Test (columns 'plan_type' or 'revenue' not found).")



print("\n I am Doing The Email Campaign Test: personalized vs non-personalized subject line") 
if {'personalized', 'converted'}.issubset(Database_After_Imputation.columns):
    conv_personal = Database_After_Imputation[Database_After_Imputation['personalized'] == 'yes']['converted'].sum()
    conv_nonpersonal = Database_After_Imputation[Database_After_Imputation['personalized'] == 'no']['converted'].sum()
    n_personal = Database_After_Imputation[Database_After_Imputation['personalized'] == 'yes'].shape[0]
    n_nonpersonal = Database_After_Imputation[Database_After_Imputation['personalized'] == 'no'].shape[0]
    z_score_mail, p_mail = proportions_ztest([conv_personal, conv_nonpersonal],
                                             [n_personal, n_nonpersonal])
    print(f"\nEmail Campaign Test: Does personalization improve open rate?\nP-value: {p_mail:.4f}")
else:
    print("\nSkipping Email Campaign Test (column 'personalized' not found).")



print("\n I am Doing The Feature Test: dark mode vs normal mode complaints Here") 
if {'theme', 'complaints'}.issubset(Database_After_Imputation.columns):
    contingency_feature = pd.crosstab(Database_After_Imputation['theme'], Database_After_Imputation['complaints'])
    chi2_feature, p_feature, dof_feature, _ = stats.chi2_contingency(contingency_feature)
    print(f"\nFeature Test: Does dark mode reduce complaints?\nP-value: {p_feature:.4f}")
else:
    print("\nSkipping Feature Test (columns 'theme' or 'complaints' not found).")
