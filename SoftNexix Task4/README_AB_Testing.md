...
It follows the steps outlined in the *"Data Analysis Using Python – Task 4"* internship assignment.
...
- **Chi-Square Test**: Checks if categorical variable (e.g., `device`) affects conversion rate.  
  *(Skipped automatically if the dataset lacks `device` column.)*
- **T-Test**: Checks if continuous metric (e.g., `session_duration`) differs between groups.  
  *(Skipped automatically if the dataset lacks `session_duration` column.)*
